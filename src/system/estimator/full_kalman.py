...
from .lpf import LowPassFiniteDifferenceEstimator, LPFParams
...
class FullStateKalmanFilter(BaseEstimator):
    ...
    def __init__(self, params: FullKalmanParams):
        ...
        self.R = np.diag([
            p.r_pose_pos, p.r_vel_pos, p.r_pose_ang, p.r_vel_ang,
            p.r_pose_pos, p.r_vel_pos, p.r_pose_ang, p.r_vel_ang,
        ])

        self._lpf = LowPassFiniteDifferenceEstimator(LPFParams(alpha=params.lpf_alpha))
